...
```console
$ helm install stable/nuget-server
```

## Introduction

This chart bootstraps an nuget-server deployment on a [Kubernetes](http://kubernetes.io) cluster using the [Helm](https://helm.sh) package manager.
...
```console
$ helm install --name my-release stable/nuget-server
```

The command deploys a nuget-server on the Kubernetes cluster in the default configuration. The [configuration](#configuration) section lists the parameters that can be configured during installation.
...
The following table lists the configurable parameters of the nuget-server chart and their default values.
...
```console
$ helm install stable/nuget-server --name my-release \
    --set api-key=e46c582041db4cbe86a84b76a374383a
```

Alternatively, a YAML file that specifies the values for the parameters can be provided while installing the chart. For example,

```console
$ helm install stable/nuget-server --name my-release -f values.yaml
```
